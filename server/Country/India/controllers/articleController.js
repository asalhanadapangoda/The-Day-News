...
const createArticle = async (req, res) => {
  try {
    ...
    const article = new Article({
      ...
      category,
      tags,
      author: author || 'The Day News Global - India',
    ...
    if (createdArticle.status === 'published') {
      pingGoogleIndexing(createdArticle.slug, 'India', 'URL_UPDATED');
    }
  ...
  try {
    ...
    if (article) {
      ...
      if (updatedArticle.status === 'published') {
        pingGoogleIndexing(updatedArticle.slug, 'India', 'URL_UPDATED');
        if (oldStatus === 'published' && oldSlug !== updatedArticle.slug) {
          pingGoogleIndexing(oldSlug, 'India', 'URL_DELETED');
        }
...
const deleteArticle = async (req, res) => {
  try {
    ...
    if (article) {
      ...
      if (status === 'published') {
        pingGoogleIndexing(slug, 'India', 'URL_DELETED');
      }
