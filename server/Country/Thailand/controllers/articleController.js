...
const createArticle = async (req, res) => {
  try {
    ...
    const article = new Article({
      ...
      category,
      tags,
      author: author || 'The Day News Global - Thailand',
    ...
    if (createdArticle.status === 'published') {
      pingGoogleIndexing(createdArticle.slug, 'Thailand', 'URL_UPDATED');
    }
  ...
  try {
    ...
    if (article) {
      ...
      if (updatedArticle.status === 'published') {
        pingGoogleIndexing(updatedArticle.slug, 'Thailand', 'URL_UPDATED');
        if (oldStatus === 'published' && oldSlug !== updatedArticle.slug) {
          pingGoogleIndexing(oldSlug, 'Thailand', 'URL_DELETED');
        }
...
const deleteArticle = async (req, res) => {
  try {
    ...
    if (article) {
      ...
      if (status === 'published') {
        pingGoogleIndexing(slug, 'Thailand', 'URL_DELETED');
      }
