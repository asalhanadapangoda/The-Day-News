...
const createArticle = async (req, res) => {
  try {
    ...
    const article = new Article({
      ...
      category,
      tags,
      author: author || 'The Day News Global - USA',
    ...
    if (createdArticle.status === 'published') {
      pingGoogleIndexing(createdArticle.slug, 'USA', 'URL_UPDATED');
    }
  ...
  try {
    ...
    if (article) {
      ...
      if (updatedArticle.status === 'published') {
        pingGoogleIndexing(updatedArticle.slug, 'USA', 'URL_UPDATED');
        if (oldStatus === 'published' && oldSlug !== updatedArticle.slug) {
          pingGoogleIndexing(oldSlug, 'USA', 'URL_DELETED');
        }
...
const deleteArticle = async (req, res) => {
  try {
    ...
    if (article) {
      ...
      if (status === 'published') {
        pingGoogleIndexing(slug, 'USA', 'URL_DELETED');
      }
