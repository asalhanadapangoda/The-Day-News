...
const createArticle = async (req, res) => {
  try {
    ...
    if (createdArticle.status === 'published') {
      pingGoogleIndexing(createdArticle.slug, 'Samoa', 'URL_UPDATED');
    }
  ...
  try {
    ...
    if (article) {
      ...
      if (updatedArticle.status === 'published') {
        pingGoogleIndexing(updatedArticle.slug, 'Samoa', 'URL_UPDATED');
        if (oldStatus === 'published' && oldSlug !== updatedArticle.slug) {
          pingGoogleIndexing(oldSlug, 'Samoa', 'URL_DELETED');
        }
...
const deleteArticle = async (req, res) => {
  try {
    ...
    if (article) {
      ...
      if (status === 'published') {
        pingGoogleIndexing(slug, 'Samoa', 'URL_DELETED');
      }
