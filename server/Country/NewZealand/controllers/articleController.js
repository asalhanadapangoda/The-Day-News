...
const createArticle = async (req, res) => {
  try {
    ...
    const article = new Article({
      ...
      category,
      tags,
      author: author || 'The Day News Global - New Zealand',
    ...
    if (createdArticle.status === 'published') {
      pingGoogleIndexing(createdArticle.slug, 'NewZealand', 'URL_UPDATED');
    }
  ...
  try {
    ...
    if (article) {
      ...
      if (updatedArticle.status === 'published') {
        pingGoogleIndexing(updatedArticle.slug, 'NewZealand', 'URL_UPDATED');
        if (oldStatus === 'published' && oldSlug !== updatedArticle.slug) {
          pingGoogleIndexing(oldSlug, 'NewZealand', 'URL_DELETED');
        }
...
const deleteArticle = async (req, res) => {
  try {
    ...
    if (article) {
      ...
      if (status === 'published') {
        pingGoogleIndexing(slug, 'NewZealand', 'URL_DELETED');
      }
