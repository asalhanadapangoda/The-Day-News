...
const Navbar = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/USA/search?q=${encodeURIComponent(searchQuery)}`);
      setSearchQuery('');
      setIsMenuOpen(false);
    }
  };
  ...
  const navLinks = [
    { name: 'Home', path: '/USA' },
    { name: 'Articles', path: '/USA/articles' },
    { name: 'About Us', path: '/USA/about' },
    { name: 'Contact', path: '/USA/contact' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to="/USA" className="flex items-center">
              <img 
                src={logo} 
