...
const Navbar = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/Samoa/search?q=${encodeURIComponent(searchQuery)}`);
      setSearchQuery('');
      setIsMenuOpen(false);
    }
  };
  ...
  const navLinks = [
    { name: 'Home', path: '/Samoa' },
    { name: 'Articles', path: '/Samoa/articles' },
    { name: 'About Us', path: '/Samoa/about' },
    { name: 'Contact', path: '/Samoa/contact' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to="/Samoa" className="flex items-center">
              <img 
                src={logo} 
