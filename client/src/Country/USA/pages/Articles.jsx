...
const Articles = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-screen">
      <SEO 
        title="US News Articles & Special Reports | THE DAY NEWS USA" 
        description="Explore in-depth articles, investigative journalism, and news reports from across the US."
        keywords="us articles, american news, Washington reporting, New York reports, the day news usa"
      />
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-500 bg-white/5 rounded-xl border border-white/10">
          No articles found for this category in the USA region.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {articles.map((article) => (
            <Link key={article._id} to={`/USA/articles/${article.slug}`} className="group glass-card overflow-hidden block flex flex-col h-full hover-glow">
