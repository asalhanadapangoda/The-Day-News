...
const Search = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-[70vh]">
      <SEO 
        title={query ? `Search: "${query}" | THE DAY NEWS USA` : 'Search News | THE DAY NEWS USA'}
        description={query ? `Search results for "${query}" on The Day News USA.` : 'Search for news articles and stories in the United States.'}
        keywords={`search, ${query ? query + ', ' : ''}us news search, articles, the day news usa`}
      />
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {results.articles.map((article) => (
                  <Link key={article._id} to={`/USA/articles/${article.slug}`} className="glass-card block h-full hover-glow transition-all overflow-hidden flex flex-col">
      ...
      {!query && (
        <div className="text-center py-20 text-gray-500">
          Enter a search term above to find content in the USA region.
        </div>
      )}
    </div>
  );
