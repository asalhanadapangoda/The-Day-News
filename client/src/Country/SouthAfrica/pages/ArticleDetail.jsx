...
const ArticleDetail = () => {
  ...
  const { data: relatedArticles = [] } = useQuery({
    queryKey: ['nz-relatedArticles', article?.category?.slug],
  ...
  if (!article) {
    return (
      <div className="text-center py-20 text-white min-h-[60vh] flex flex-col justify-center items-center">
        ...
        <Link to="/SouthAfrica/articles" className="text-primary hover:underline">Return to Articles</Link>
      </div>
    );
  }
  ...
  return (
    <div className="pb-20">
      <SEO 
        title={article.metaTitle ? article.metaTitle : `${article.title} | THE DAY NEWS SOUTH AFRICA`}
        description={article.metaDescription || article.excerpt || (article.content ? article.content.replace(/<[^>]+>/g, '').slice(0, 160) : '')}
        keywords={article.metaKeywords || `south africa news, ${article.category?.name || 'article'}, breaking news`}
      ...
      <div className="w-full relative min-h-[40vh] md:min-h-[60vh] flex items-end">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 w-full pb-10">
          {article.category && (
            <Link to={`/SouthAfrica/articles?category=${article.category.slug}`} className="inline-block px-3 py-1 bg-primary text-white text-xs font-bold uppercase tracking-widest rounded mb-6 hover:bg-white hover:text-primary transition-colors">
          ...
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedArticles.map(rel => (
                <Link key={rel._id} to={`/SouthAfrica/articles/${rel.slug}`} className="group glass-card overflow-hidden block hover-glow flex flex-col h-full">
