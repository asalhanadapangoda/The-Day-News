...
const Articles = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-screen">
      <SEO 
        title="South African Articles & Reports | THE DAY NEWS SOUTH AFRICA" 
        description="Read top news articles, commentary, and in-depth stories from South Africa."
        keywords="south africa articles, sa news, johannesburg reports, the day news south africa"
      />
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-500 bg-white/5 rounded-xl border border-white/10">
          No articles found for this category in the SouthAfrica region.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {articles.map((article) => (
            <Link key={article._id} to={`/SouthAfrica/articles/${article.slug}`} className="group glass-card overflow-hidden block flex flex-col h-full hover-glow">
