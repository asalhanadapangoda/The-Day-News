...
const Navbar = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/Thailand/search?q=${encodeURIComponent(searchQuery)}`);
      setSearchQuery('');
      setIsMenuOpen(false);
    }
  };
  ...
  const navLinks = [
    { name: 'Home', path: '/Thailand' },
    { name: 'Articles', path: '/Thailand/articles' },
    { name: 'About Us', path: '/Thailand/about' },
    { name: 'Contact', path: '/Thailand/contact' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to="/Thailand" className="flex items-center">
              <img 
                src={logo} 
