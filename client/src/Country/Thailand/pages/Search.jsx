...
const Search = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-[70vh]">
      <SEO 
        title={query ? `Search: "${query}" | THE DAY NEWS THAILAND` : 'Search News | THE DAY NEWS THAILAND'}
        description={query ? `Search results for "${query}" on The Day News Thailand.` : 'Search for news articles and stories in Thailand.'}
        keywords={`search, ${query ? query + ', ' : ''}thailand news search, articles, the day news thailand`}
      />
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {results.articles.map((article) => (
                  <Link key={article._id} to={`/Thailand/articles/${article.slug}`} className="glass-card block h-full hover-glow transition-all overflow-hidden flex flex-col">
      ...
      {!query && (
        <div className="text-center py-20 text-gray-500">
          Enter a search term above to find content in the Thailand region.
        </div>
      )}
    </div>
  );
