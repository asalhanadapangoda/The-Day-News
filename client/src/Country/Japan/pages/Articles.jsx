...
const Articles = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-screen">
      <SEO 
        title="Japanese News Articles | THE DAY NEWS JAPAN" 
        description="Explore articles, culture stories, and breaking news from Japan."
        keywords="japan articles, tokyo news, japanese reporting, the day news japan"
      />
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-500 bg-white/5 rounded-xl border border-white/10">
          No articles found for this category in the Japan region.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {articles.map((article) => (
            <Link key={article._id} to={`/Japan/articles/${article.slug}`} className="group glass-card overflow-hidden block flex flex-col h-full hover-glow">
