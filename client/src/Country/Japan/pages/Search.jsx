...
const Search = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-[70vh]">
      <SEO 
        title={query ? `Search: "${query}" | THE DAY NEWS JAPAN` : 'Search News | THE DAY NEWS JAPAN'}
        description={query ? `Search results for "${query}" on The Day News Japan.` : 'Search for news articles and stories in Japan.'}
        keywords={`search, ${query ? query + ', ' : ''}japan news search, articles, the day news japan`}
      />
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {results.articles.map((article) => (
                  <Link key={article._id} to={`/Japan/articles/${article.slug}`} className="glass-card block h-full hover-glow transition-all overflow-hidden flex flex-col">
      ...
      {!query && (
        <div className="text-center py-20 text-gray-500">
          Enter a search term above to find content in the Japan region.
        </div>
      )}
    </div>
  );
