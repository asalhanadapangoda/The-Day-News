...
const Home = () => {
  ...
  const { data: settings } = useQuery({ 
    queryKey: ['nz-settings'], 
    queryFn: () => api.get('/settings').then(res => res.data),
    staleTime: 1000 * 60 * 60, // 1 hour
  });
  const { data: articles = [] } = useQuery({ 
    queryKey: ['nz-articles'], 
    ...
    <div className="w-full">
            ...
            <div className="absolute bottom-0 left-0 w-full p-6 md:p-16 max-w-7xl mx-auto flex flex-col justify-end h-full">
              <div className="max-w-3xl space-y-4 md:space-y-6">
                ...
                <h1 className="text-2xl md:text-6xl font-bold text-white leading-tight text-glow">
                  Welcome to The Day News Global - New Zealand
                </h1>
                <p className="text-sm md:text-xl text-gray-300 line-clamp-3 md:line-clamp-none">
                  Your trusted media partner for credible, engaging, and informative news coverage across Aotearoa and beyond.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="flex flex-wrap justify-between items-end gap-4 mb-10 border-b border-white/10 pb-4">
          ...
          <Link to="/NewZealand/articles" className="text-primary hover:text-white transition-colors flex items-center gap-1 text-sm uppercase tracking-wider font-semibold whitespace-nowrap">
        ...
        {recentArticles.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {recentArticles.map(article => (
              <Link key={article._id} to={`/NewZealand/articles/${article.slug}`} className="group bg-[#1a1a1a] rounded-xl overflow-hidden shadow-lg border border-white/5 hover:border-primary/50 transition-colors flex flex-col h-full">
          ...
          <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/10">
            <p className="text-gray-500 italic">No articles found. Stay tuned for new updates from New Zealand.</p>
          </div>
        )}
      </section>
