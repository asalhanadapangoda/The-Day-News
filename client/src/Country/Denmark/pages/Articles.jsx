...
const Articles = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-screen">
      <SEO 
        title="Danish Articles & Coverage | THE DAY NEWS DENMARK" 
        description="Explore the latest articles, analysis, and breaking stories from Denmark."
        keywords="denmark articles, danish reporting, copenhagen news, the day news denmark"
      />
      ...
      <div className="flex flex-wrap justify-center gap-3 mb-12">
        <button
          ...
          className={`px-5 py-2 rounded-full text-sm font-semibold transition-colors border ${currentCategory === ''
        ...
        {categories.map(cat => (
          <button
            ...
            className={`px-5 py-2 rounded-full text-sm font-semibold transition-colors border ${currentCategory === cat.slug
              ? 'bg-primary border-primary text-white'
              : 'bg-transparent border-white/20 text-gray-300 hover:border-primary hover:text-white'
              }`}
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-500 bg-white/5 rounded-xl border border-white/10">
          No articles found for this category in the Denmark region.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {articles.map((article) => (
            <Link key={article._id} to={`/Denmark/articles/${article.slug}`} className="group glass-card overflow-hidden block flex flex-col h-full hover-glow">
