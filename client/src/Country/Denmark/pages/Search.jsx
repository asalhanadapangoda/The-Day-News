...
const Search = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-[70vh]">
      <SEO 
        title={query ? `Search: "${query}" | THE DAY NEWS DENMARK` : 'Search News | THE DAY NEWS DENMARK'}
        description={query ? `Search results for "${query}" on The Day News Denmark.` : 'Search for news articles and stories in Denmark.'}
        keywords={`search, ${query ? query + ', ' : ''}denmark news search, articles, the day news denmark`}
      />
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {results.articles.map((article) => (
                  <Link key={article._id} to={`/Denmark/articles/${article.slug}`} className="glass-card block h-full hover-glow transition-all overflow-hidden flex flex-col">
      ...
      {!query && (
        <div className="text-center py-20 text-gray-500">
          Enter a search term above to find content in the Denmark region.
        </div>
      )}
    </div>
  );
