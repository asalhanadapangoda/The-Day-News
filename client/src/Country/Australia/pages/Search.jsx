...
const Search = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-[70vh]">
      <SEO 
        title={query ? `Search: "${query}" | THE DAY NEWS AUSTRALIA` : 'Search News | THE DAY NEWS AUSTRALIA'}
        description={query ? `Search results for "${query}" on The Day News Australia.` : 'Search for news articles and video stories in Australia.'}
        keywords={`search, ${query ? query + ', ' : ''}australia news search, articles, the day news australia`}
      />
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {results.programs.map((program) => (
                  <Link key={program._id} to={`/Australia/programs/${program.slug}`} className="group glass-card overflow-hidden block">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {results.episodes.map(episode => (
                  <Link key={episode._id} to={`/Australia/programs/${episode.program?.slug || ''}`} className="glass-card flex p-4 gap-4 hover-glow transition-all">
                    <OptimizedImage 
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {results.articles.map((article) => (
                  <Link key={article._id} to={`/Australia/articles/${article.slug}`} className="glass-card block h-full hover-glow transition-all overflow-hidden flex flex-col">
